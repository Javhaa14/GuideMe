...
// Guide Profile Form Component
function GuideProfileForm({ initialData }: { initialData?: any }) {
  const { user } = useUser();
  ...
  const onSubmit = async (values: GuideProfile) => {
    try {
...
// Tourist Profile Form Component
function TouristProfileForm({ initialData }: { initialData?: any }) {
  const { user } = useUser();
  const router = useRouter();
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [languageOptions, setLanguageOptions] = useState<
  ...
  const onSubmit = async (values: TouristProfile) => {
    try {
...
// Main component that renders the appropriate form based on type
export function UnifiedProfileForm({
  type,
  initialData,
}: UnifiedProfileFormProps) {
  if (type === "guide") {
    return <GuideProfileForm initialData={initialData} />;
  } else {
    return <TouristProfileForm initialData={initialData} />;
  }
}
