...
import { guideProfileModel } from '../model/GuideProfile';

export const createGuideProfile = async (req: Request, res: Response) => {
  try {
    const guideProfile = await guideProfileModel.create(req.body);
...
export const getGuideProfile = async (_req: Request, res: Response) => {
  try {
    const guideProfile = await guideProfileModel.find();
