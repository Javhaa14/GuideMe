...
export const updateGuideProfile = async (
  ...
  try {
    ...
    if (alreadyLiked) {
      // Unlike хийсэн бол хасна
      guideProfile.likedBy = guideProfile.likedBy.filter(
        (id) => id.toString() !== userId
      );
    } else {
      // Шинээр like хийсэн бол
      guideProfile.likedBy.push(new mongoose.Types.ObjectId(userId));

      // ✅ Notification үүсгэх
      if (guideProfile._id && guideProfile._id.toString() !== userId) {
        await Notification.create({
          userId: guideProfile._id, // Хэн рүү notification очих
          fromUserId: userId, // Хэн like хийсэн
          type: "like",
